...
var watchDir = function watchDir(dirpath, stream, opts) {
  ...
  var onChange = function onChange(name) {
    ...
    stream.next('changed', _path2['default'].relative(opts.base, file), 'from = ' + _path2['default'].relative(opts.base, dirpath));
  };
  var onRename = function onRename(name) {
    var file = _path2['default'].join(dirpath, name);
    _fs2['default'].exists(file, function (e) {
      var type = e ? 'added' : 'deleted';
      stream.next(type, _path2['default'].relative(opts.base, file), 'from = ' + _path2['default'].relative(opts.base, dirpath));
...
pryPath = function (file, stream, opts) {
  ...
  _fs2['default'][statFn](file, function (err, stats) {
    ...
    if (opts.walk) {
      stream.next('visited', _path2['default'].relative(opts.base, file));
    }
    if (stats.isDirectory()) {
      if (opts.watch) {
        watchDir(file, stream, opts);
